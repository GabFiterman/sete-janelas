          ...
          windows: windows.map((window) => {
            if (window.id === newWindow.id) {
              return {
                ...
                zIndex: newZIndex,
                status: isMobileDevice || newWindow.appName === 'AcrobatReader' ? 'maximized' : (newWindow.status ?? window.status),
              };
            }
            return window;
          }),
      ...
      return {
        windows: [
          ...state.windows,
          {
            ...
            status: isMobileDevice || newWindow.appName === 'AcrobatReader' ? 'maximized' : (newWindow.status ?? 'normal'),
